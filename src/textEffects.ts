...
type Variant = "m" | "b" | "i" | "c" | "g" | "d" | "s" | "o" | "p" | "q" | "w";
type Flags = "underline" | "strike" | "u,s";

function applyTextEffect(str: string, variant?: Variant, flags?: Flags) {
  const offsets: any = {
    m: [0x1d670, 0x1d7f6],
    b: [0x1d400, 0x1d7ce],
    i: [0x1d434, 0x00030],
  ...
  const getType = function (variant: any) {
    if (variantOffsets[variant]) return variantOffsets[variant];
    if (offsets[variant]) return variant;
    return "m"; //monospace as default
  };
  const getFlag = function (flag: any, flags: any) {
    if (!flags) return false;
    return flag.split("|").some((f: any) => flags.split(",").indexOf(f) > -1);
  };
...
export { Variant, applyTextEffect };
